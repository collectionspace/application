...
public class WebUI implements CSP, UI, Configurable {
	...
	public String getLoginFailedDest() {
		return login_failed_dest;
	}
	 ...
	 * would open up the url /chain/logout for GET method with no parameters on
	 * the url and call WebLogout.java
	 * 
	 * addMethod(Operation.READ,new String[]{r.getWebURL()},1,new
	 * RecordRead(r));
	 * would open up the url /chain/<web-url>/{parameter} where web-url is the
	 * name in cspace-config.xml
	 * expecting one parameter and calling RecordRead.java and passing the
	 * record info from cspace-config.xml
	 * 
	 * @param op
	 *            - GET/POST that request came in as
	 *            Operation.CREATE = POST
	 *            Operation.READ = GET
	 * @param path
	 *            - url this json came in via e.g. /chain/login/ (remove the
	 *            /chain/ part)
	 * @param extra
	 *            - does this url have extra info e.g. /chain/object/1234{id}
	 *            0=no, 1= one item 2= two etc
	 * @param method
	 *            - Java class to call to process this request
	 */
		...
		rules.addRule(SECTIONED, new String[] { "ui", "web" }, SECTION_PREFIX + "web", null, new RuleTarget() {
			...
			public Object populate(Object parent, ReadOnlySection section) {
				...
				if (section.getValue("/tmp-schema-path") != null) {
					uispec_path = System.getProperty("java.io.tmpdir") + "/ju-cspace"; // XXX
																						// fix
				} else {
	...
	private void configure_finish(Spec spec) {
		...
		for (Record r : spec.getAllRecords()) {
			...
			if (r.isType("authority")) {
				...
				// We need to do a special case for related media. Weird, but
				// makes a certain sense...
				// We do not actually care about the type of r, but rather just
				// the CSID tail.
				...
				// addMethod(Operation.READ,new
				// String[]{r.getWebURL(),"procedures"},1,new RecordRelated(r));

				// All the relatable records need to be able to fetch the other
				// relatable records.
